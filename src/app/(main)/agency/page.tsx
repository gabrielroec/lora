  ...
  if (agencyId) {
    if (user?.role === "SUBACCOUNT_GUEST" || user?.role === "SUBACCOUNT_USER") {
      ...
      if (searchParams.state) {
        ...
        if (!stateAgencyId) return <div>Não autorizado</div>;

        return redirect(`agency/${stateAgencyId}/${statePath}?code=${searchParams.code}`);
      } else return redirect(`/agency/${agencyId}`);
    } else return <div>Não autorizado</div>;
  }
  ...
  return (
    <div className="flex justify-center items-center mt-4">
      <div className="max-w-[850px] border p-4 rounded-xl">
        <h1 className="text-2xl font-bold">Seja bem-vindo(a) à sua nova agência!</h1>
        <p className="text-sm text-gray-500">Para começar, você precisa escolher um plano de assinatura.</p>
